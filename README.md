...
- **Lando + asdf** (existing workflow)
- **Devbox** (portable Linux/macOS workflow using `devbox.json`)

### Option A: Lando (current workflow)
...
### Option B: Devbox (portable on Linux/macOS)
...
1. Check out code and `cd` into the repo
...
7. Access application at [http://localhost:3000/](http://localhost:3000)

##### Running Sidekiq (devbox)
...
```bash
devbox run services-down
```

> Notes on devbox database connectivity

- Devbox runs Postgres bound to a **unix socket** under `.postgres/` (not TCP localhost by default).

- `config/database.yml` is compatible with both Lando and Devbox by checking Lando env vars first, then `PGHOST/PGPORT/PGUSER/PGPASSWORD`.
