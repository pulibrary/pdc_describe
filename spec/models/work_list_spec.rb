...
RSpec.describe WorkList, type: :model do
  ...
  describe "#unfinished_works" do
    before do
      ...
      WorkActivity.add_work_activity(ds.id, "Tagging @#{user.uid} in this dataset", rd_user.id)
    end
  ...
  describe "#completed_works" do
    before do
      ...
      FactoryBot.create(:draft_work, created_by_user_id: user.id)
      FactoryBot.create(:approved_work, created_by_user_id: pppl_user.id, collection_id: Collection.plasma_laboratory.id)
      # Create the dataset for `rd_user` and @mention `user`
  ...
  describe "#withdrawn_works" do
    before do
      ...
      FactoryBot.create(:awaiting_approval_work, created_by_user_id: user.id)
      FactoryBot.create(:draft_work, created_by_user_id: user.id)
      pppl_work = FactoryBot.create(:approved_work, created_by_user_id: pppl_user.id, collection_id: Collection.plasma_laboratory.id)
