...
RSpec.describe Collection, type: :model do
  it "creates default collections only when needed" do
    ...
    expect(Group.where(code: "PPPL").count).to be 1
    expect(Group.where(code: "RD").count).to be 1
  ...
  it "creates defaults when not defined" do
    described_class.delete_all
    expect(described_class.count).to be 0
    expect(Group.default).to_not be nil

    described_class.delete_all
    expect(described_class.count).to be 0
    expect(Group.default_for_department("41000")).to_not be nil
  end
