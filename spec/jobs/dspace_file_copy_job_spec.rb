...
RSpec.describe DspaceFileCopyJob, type: :job do
  ...
  before do
    ...
    allow(fake_s3_service).to receive(:copy_directory).and_return(fake_completion)
  ...
  context "when the files is a directory" do
    subject(:job) { described_class.perform_later("10.34770/ackh-7y71", "10-34770/ackh-7y71/dir/", 0, 1) }
    it "copies directory" do
      perform_enqueued_jobs { job }
      expect(fake_s3_service).to have_received(:copy_directory).with(source_key: "example-bucket-dspace/10-34770/ackh-7y71/dir/",
                                                                     target_bucket: "work-bucket",
                                                                     target_key: "abc/123/#{work.id}/dir/")
    end
  end
end
