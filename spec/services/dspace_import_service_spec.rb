...
describe DspaceImportService do
  ...
  describe "#import!" do
    ...
    it "creates a new Work from imported Dublin Core XML metadata" do
      ...
      expect(imported.dublin_core.language).to include("en_US")

      expect(imported.dublin_core.relation).to be_an(Array)
      expect(imported.dublin_core.relation).to include("PandemicImpactReport.com")

      expect(imported.dublin_core.publisher).to be_an(Array)
      expect(imported.dublin_core.publisher).to include("Cambridge")
      expect(imported.dublin_core.publisher).to include("Harvard Medical School")
    end
  end
end
