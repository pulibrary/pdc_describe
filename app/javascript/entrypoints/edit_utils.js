...
var rolesHtml = function(roleId, activeRole) {
  var i, html, roles, key, value, selected;
  roles = contributorRoles();
  html = `<select id="${roleId}" name="${roleId}">`;
  for(i = 0; i < roles.length; i++) {
    key = roles[i].key;
    value = roles[i].value;
    selected = activeRole == key ? "selected" : "";
...
var addRelatedObjectHtml = function(num, related_identifier, related_identifier_type, relation_type) {
  ...
  var relatedIdentifierTypeHtml = relatedIdentififerTypeSelectHtml(relatedIdentifierTypeId, related_identifier_type);
  var relationTypeHtml = relationTypeSelectHtml(relationTypeId, relation_type);
...
var relatedIdentififerTypeSelectHtml = function(relatedIdentifierTypeId, related_identifier_type) {
  var i, html, key, value, selected;
  const types = relatedIdentifierTypes();
  html = `<select id="${relatedIdentifierTypeId}" name="${relatedIdentifierTypeId}">`;
  for(i = 0; i < types.length; i++) {
    key = types[i].key;
    value = types[i].value;
    selected = related_identifier_type == key ? "selected" : "";
    html += `<option value="${key}" ${selected}>${value}</option>`;
  }
  html += `</select>`;
  return html;
...
var relationTypeSelectHtml = function(relationTypeId, relation_type) {
  var i, html, key, value, selected;
  const types = relationTypes();
  html = `<select id="${relationTypeId}" name="${relationTypeId}">`;
  for(i = 0; i < types.length; i++) {
    key = types[i].key;
    value = types[i].value;
    selected = relation_type == key ? "selected" : "";
    html += `<option value="${key}" ${selected}>${value}</option>`;
  }
  html += `</select>`;
  return html;
...
var addContributorHtml = function(num, orcid, givenName, familyName, role, sequence) {
  ...
  var roleHtml = rolesHtml(roleId, role);
